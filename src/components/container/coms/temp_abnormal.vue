<template>
  <div class='temp-abnormal-tiles-wrap'>
  ...
  methods: {
    getMeterAbnormalSum () {
      ...
      getMeterAbnormalNum().then((data) => {
        if (_.get(data, '[5].sum')) {
          let temp = _.find(data, (item) => {
            return item.name === '温度异常'
          })
          this.sum = _.get(temp, 'sum', 0)
