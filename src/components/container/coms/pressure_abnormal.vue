<template>
  <div class='pressure-abnormal-tiles-wrap'>
  ...
  methods: {
    getMeterAbnormalSum () {
      ...
      getMeterAbnormalNum().then((data) => {
        if (_.get(data, '[5].sum')) {
          let pressure = _.find(data, (item) => {
            return item.name === '压力异常'
          })
          this.sum = _.get(pressure, 'sum', 0)
