<template>
  <div class='batter-abnormal-tiles-wrap'>
  ...
  methods: {
    getMeterAbnormalSum () {
      ...
      getMeterAbnormalNum().then((data) => {
        if (_.get(data, '[5].sum')) {
          let batter = _.find(data, (item) => {
            return item.name === '电池电量异常'
          })
          this.sum = _.get(batter, 'sum', 0)
